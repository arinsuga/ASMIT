$(document).ready(function() {
    ...
    //Event OnChange
    $('#fldASST_STS_RUID').change(function() {
        //alert("berubah nih : " + $('#fldASST_STS_RUID').val());
    ...
    $('#LKP_PROD_ITMNO').click(function() {
        ...
        vsSearchValue = new Array();
        vsSearchValue[0] = "";
        showLookup('LKP_ASMTRN_STOCKIN_BYSN_CSTM01', '#imgPROD_ITMNO', vsSearchValue, '#fldSTOCKIN_RUID', '#fldPROD_ITMNO', '');
    ...
    //Command LookUp
    $('#LKP_ASST_STS_RUID').click(function() {
        vsSearchValue = new Array();
        vsSearchValue[0] = "";
        showLookup('LKP_ASMLOV_ASST_STS', '#imgASST_STS_RUID', vsSearchValue, '#fldASST_STS_RUID', '', '#fldASST_STS_RUIDx');
    });
    ...
    //Command LookUp
    $('#LKP_RSRC_CTR_PRTY_RUID').click(function() {
        //alert();
        vsSearchValue = new Array();
        vsSearchValue[0] = "";
        showLookup('LKP_RESOURCE_SAPVENDOR', '#imgRSRC_CTR_PRTY_RUID', vsSearchValue, '#fldRSRC_CTR_PRTY_RUID', '', '#fldRSRC_CTR_PRTY_RUIDx');
        //Only for testing ==> showLookup('LKP_RESOURCE', vsSearchValue, '#fldRUID', '#fldPROD_ITMNO', '');
    });
            ...
            for (i = 0; i < vnRowCount; i++) {
                if (aStatusList[i] == psAssetStatus) {
                    ...
                    if (afldRSRC_CTR_PRTY_RUIDIsBlank[i] == "Y") {
                        ...
                        $("#fldRSRC_CTR_PRTY_RUIDx").val("");
                    } else {
                        $("#fldRSRC_CTR_PRTY_RUID").val(voFormDataOld.fldRSRC_CTR_PRTY_RUID);
                        showLookupInline("MST_RSRC", "#imgRSRC_CTR_PRTY_RUID", voFormDataOld.fldRSRC_CTR_PRTY_RUID, "#fldRSRC_CTR_PRTY_RUIDx");
                    } //End If
                    break;
                } //End If
            } //End For
    ...
    function setMandatoryByAssetStatus(psAssetStatus) {
        clearMandatoryByAssetStatus();
        $("#fldASST_STS_RUID").addClass("FieldAsMandatory");
        $("#fldTRN_DT").addClass("FieldAsMandatory");
        $("#fldDOC_REFF_NO").addClass("FieldAsMandatory");

        if (psAssetStatus == "LOV_ASST_STS_HANDOVER") {
            $("#fldASST_TAG_ID").addClass("FieldAsMandatory");
            $("#fldDEPT_RUID").addClass("FieldAsMandatory");
            $("#fldRSRC_USER_RUID").addClass("FieldAsMandatory");
        }
        if (psAssetStatus == "LOV_ASST_STS_USED") {
            $("#fldASST_TAG_ID").addClass("FieldAsMandatory");
        }
    ...
    function setFormValueOfStockOut(poMsg) {
        ...
        showLookupInline("ASM_ASST_STS", "#imgASST_STS_RUID", poMsg.fldASST_STS_RUID, "#fldASST_STS_RUIDx");
        ...
        showLookupInline("MST_RSRC", "#imgRSRC_CTR_PRTY_RUID", poMsg.fldRSRC_CTR_PRTY_RUID, "#fldRSRC_CTR_PRTY_RUIDx");
    ...
    function doSaveData() {
        var vReturn = "Y";
        var vsUrl = "frmStockOut.asmx/doSaveData";
        var voDTO = DTO;
    ...
    function doInsertData() {
        var vReturn = false;
        var vsUrl = "frmStockOut.asmx/doInsertData";
        var voDTO = DTO;
        $('#DialogSaveOnProgress').dialog('open');
        $.ajax({
            type: getAJAXType1(),
    ...
    function doUpdateData() {
        var vReturn = false;
        runAJAX_CRUD("frmStockOut.asmx/doUpdateData", DTO);
        return vReturn;
    }
    //Function Update Data
    function doDeleteData() {
        var vReturn = false;
        runAJAX_CRUD("frmStockOut.asmx/doDeleteData", DTO);
        return vReturn;
    }
